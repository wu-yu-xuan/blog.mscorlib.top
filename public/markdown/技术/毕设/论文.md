...
该网站采用 React 框架进行开发, 同时使用了 pwa 与并发渲染等前沿技术,
数据表明, 一方面, 学生在实验过程中提高了自己的硬件知识与学习兴趣, 另一方面, 网站的响应速度较快, 故障率较低, 具有极佳的用户体验.

## Abstract

a remote virtual expreiment platform for FPGA was designed and developed. this platform uses react frontend framework with advanced technology such as pwa and cocurrent mode. The data shows that on the one hand, students have improved their hardware knowledge and interest during the experiment. On the other hand, the platform has a faster response speed, a lower failure rate, and an excellent user experience.
...
因此, 建设远程虚拟实验平台无疑能为高校教学活动带来极大的便利. 本文依托现有实验室的 FPGA 实验设备, 开发了远程 FPGA 虚拟实验平台网站, 并就其设计思路与开发过程进行了讨论.

## 设计

### 安全设计

日益复杂的网络环境对高校实验平台提出了新的挑战[10], 要放在首要位置考虑的就是安全性, 安全性主要分为以下两点:

#### 防止剽窃成果

一方面, 每天互联网上都有大量爬虫在运行[11], 另一方面, 前端的特性注定了它的代码是公开的, 那么如何防止他人爬取前端代码呢?

首先使用代码压缩与混淆工具将其压缩混淆, 使其丧失可读性, 就算他人获取了代码, 也无法分析阅读, 其次, 启用同源策略, 拒绝跨域请求, 使代码仅可以运行在特定域名上, 使攻击者无法有效的运行代码.

#### 防止代做实验

考虑到可能有攻击者会接受同学代做实验的请求, 所以笔者的解决方法是每次请求都使用 AES-256[12] 的公钥加密, 传递到服务端后, 再由服务端使用对应的私钥解谜, 在这种情况下, 攻击者截获了流量也无法得知其中的信息是什么, 更别说伪造流量了.
...
数据表明, 学生的硬件知识水平与动手能力得到了一定的提高, 教师的教学压力也得到了分担.
未来, 本实验平台还会在 FPGA 实验的基础上增加电气, 机械, 医疗等等专业的有关实验, 为不同的专业提供针对性的解决方案, 实现实验环境的一体化.
...
[10] 张迎, 宁玉文, 高东怀, et al. 高校网站信息安全威胁与对策探析[J]. 中国教育信息化(09):30-33.
[11] Kevin. 网络爬虫技术原理[J]. 计算机与网络, 2018, v.44；No.578(10):41-43.
[12] Feldhofer, M, Wolkerstorfer, J, Rijmen, V. AES implementation on a grain of sand[J]. IEE Proceedings - Information Security, 152(1):13.
